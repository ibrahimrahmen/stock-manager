...
BOT_SYSTEM_PROMPT_AR = (
    ...
    "('fih', 'taille', 'livraison', 'disponible').\n"
    ...
    "'nwaslou l tunis el kol'. Ki t7eb tos2el 3al mntej 9oll barka: 'ama article "
    ...
    "   'Aslema khouya, hedha [ESM EL MNTEJ] b [PRIX] DT w livraison 7 DT. "
    ...
    "PRIX — TARTIB SARIM (ma tkhalfouch):\n"
    ...
    "'sa77a' barka (acknowledgement, mouch so2el, mouch mntej, mouch adresse/"
    "noumrou): MA TRePONDICH — okteb barka: SKIP.\n"
    "- AMA MOHIM: kén fama SO2EL (fih '?' wala kelma ki 'disponible', "
    "'mawjoud', 'famma', '9adeh', 'kifeh', 'chnowa', 'wa9tech', 'taille', "
    "'couleur'...) MA TA 3MELCH SKIP — LEZEM TJAWEB. 'SKIP' huwa barka lel "
    "kelmet el fer3a (ok/merci) eli ma fihomch so2el.\n"
    "- KI YES2EL 3AL DISPONIBILITE ('disponible ?', 'mawjoud ?', 'famma "
    "stock ?', 'famma ?'): jaweb KOURT b jomla wa7da: 'aah khouya disponible, "
    "[el mntej] b [prix] DT + 7 DT livraison'. MOHIM BARCHA: MA TZIDCH "
    "taille/couleur/adresse/noumrou wala 'ab3athelna...' — el 7arif sa2el 3al "
    "disponibilité BARKA, ma 9allekch eli 7ab ya3mel commande. Info el "
    "commande tetleb KEN wa9t el 7arif ywerri eli 7ab yechri (9allek 'n7eb', "
    "'na3mel commande', wala 3tak taille/adresse).\n"
    ...
    "COMMANDE — QAWAAID MOHIMA:\n"
    "- ESM EL 7ARIF: MA TES2ELCH 3lih — yji otomatik mel profil Facebook. "
    "Es2el barka: taille, couleur (kén mahich fel taswira), noumrou, adresse.\n"
    "- NOUMROU: lzmou 8 chiffres (tounsi). Kén el noumrou mouch 8 chiffres, "
    "9ollou y2akedlek el noumrou s7i7 9bal ma tkammel.\n"
    "- COULEUR MEL TASWIRA: kén el 7arif ba3ath taswira, e9ra el couleur MEL "
    "TASWIRA (taswira ak7la = ak7al). MA TES2ELCH 3al couleur eli deja bayna "
    "fel taswira. Es2el 3al couleur barka kén ma famech taswira wala mouch "
    "wadhla.\n"
    "- MELANGE EL COULEURS: chouf el produits eli fel offre. Offre b produit "
    "WA7ED (ensemble mrekeb) = MA TNAJJEMCH tbadel, couleur wa7da lel ensemble "
    "kamel. Offre b ZOUZ produits mnfaslin (haut + bas) = el 7arif ynajjem "
    "ykhalet: couleur el haut w couleur el bas kol wa7ed mel variantes mte3ou.\n"
    "- ZOUZ COMMANDE ('les deux', 'fihom zouz'): el 7arif 7ab EL ZOUZ offres "
    "maa baadhhom — zidhom fel commande, MA TBADELCH wa7da b okhra.\n"
    "- BADEL RAY (change of mind): kén el 7arif ba3ath taswira JDIDA wala 9allek "
    "b wodhou7 mntej akhor, 3awed t3aref el mntej ejjdid w badel el mntej. Ama "
    "kelma taysa (mathal kelma fel esm el pub) MA TBADELCH biha el mntej.\n"
    "- DÉCOR MOUCH FEL OFFRE: 7wayej décor fel taswira (claquette, cintre, "
    "carte) mch mel produit — MA TZIDHOMCH fel commande.\n"
    "- TETFA9ED 9BAL EL5ALS: ei — Barats boutique en ligne, khlas 3and "
    "el istilem, w el 7arif ynajjem y7el w yetfa9ed el colis 9bal ma y5ales.\n"
    "- KÉN 3ANDEK noumrou (8 chiffres) + adresse + mntej: 9oll 'wselna tolbek, "
    "el equipe bech tkalmek 3al confirmation' w kammel.\n\n"
...
def _match_product_by_image_once(local_images, url_images, offers_data, model=None):
    ...
    try:
        ...
        pick_prompt = (
            "Regarde la PHOTO ci-jointe (ce que le client a envoyé). "
            "Description de secours:\n" + seen
            + "\n\nProduits candidats du catalogue:\n" + clist
            + "\n\nTrouve le numéro qui est le MÊME produit que la photo.\n"
            "RÈGLE PRINCIPALE — identifie par la CARACTÉRISTIQUE DISTINCTIVE: "
            "le MOTIF/imprimé exact et sa forme, un détail SIGNATURE, ou un "
            "logo/marque. Exemples: jacquard monogramme rose = Premium Casa; "
            "rayures navy sur crème = Casablanca; motif camouflage/cow-print = "
            "Camo; tricot torsadé (cable) = Vintage; côtelé (ribbed) + demi-zip "
            "= SORA. Si la caractéristique distinctive de la photo correspond "
            "nettement à UN candidat, CHOISIS-le.\n"
            "TOLÈRE ces différences (NE rejette PAS pour ça): (a) la COULEUR — "
            "un même produit existe en plusieurs couleurs, la couleur est "
            "choisie séparément; (b) ne voir qu'une PARTIE d'un ensemble (ex: 2 "
            "pièces d'un 3-pièces, ou juste le haut); (c) une marque MAL LUE ou "
            "absente (ZNA≈ZARA); (d) col ou manches un peu différents; "
            "(e) accessoires/props (casquette, chaussures, cintre).\n"
            "SAISON (hauts seulement): un pull/veste manches longues épais = "
            "hiver; un t-shirt/polo léger = été — ne choisis pas un HAUT de la "
            "mauvaise saison. Pour pantalon/jogging/short/chaussures, IGNORE la "
            "saison [crochets].\n"
            "NE FORCE PAS: si AUCUN candidat n'a la même caractéristique "
            "distinctive (le même motif/design), réponds 0 — mieux vaut 0 "
            "(l'équipe vérifie) qu'un mauvais produit. MAIS si le motif/design "
            "distinctif correspond nettement, CHOISIS (ne rejette pas juste "
            "pour une couleur, une pièce manquante, une marque mal lue ou un "
            "col).\n"
            "Réponds UNIQUEMENT par le numéro (ex: 3), ou 0 si aucun design ne "
            "colle."
        )
        pick_prompt2 = (pick_prompt
            + "\n\nFormat: le numéro, une virgule, puis 'sur' si la "
            "caractéristique distinctive colle nettement, ou 'pasur' si "
            "plusieurs candidats ont le même motif et tu hésites vraiment. "
            "Ex: '3,sur' ou '5,pasur'.")
...
def api_offer_detail(request, offer_id):
    ...
    from .models import Offer
    try:
        offer = Offer.objects.prefetch_related("images", "products__product__variants__units").get(pk=offer_id)
    ...
    return JsonResponse({
        ...
        "offer": {
            ...
            "offer_images": [{
                "id": oi.id, "color_name": oi.color_name,
                "color_label": oi.color_label,
                "url": (oi.image.url if oi.image else ""),
...
def _create_order_from_shopify_shaped_payload(payload, source="shopify", external_id="", request=None, sales_page_id=None):
    ...
    # "Commande reçue" SMS only for website channels (Shopify / Converty).
    # Messenger & Instagram customers are already talking to us in the DM
    # conversation, so we skip this one for them (they still get the
    # injoignable / expédié / en cours SMS via _maybe_send_status_sms).
...
def api_offer_create(request):
    ...
    with transaction.atomic():
        ...
        _save_offer_images(request, offer)
        if page_ids:
