...
def _match_product_by_image_once(local_images, url_images, offers_data, model=None):
    ...
    try:
        ...
        pick_prompt = (
            ...
            "1) D'ABORD la SAISON (SEULEMENT pour les HAUTS): un HAUT à MANCHES "
            "LONGUES / tissu ÉPAIS (pull, sweat, veste) = HIVER ; à MANCHES "
            "COURTES / tissu léger (t-shirt, polo) = ÉTÉ. Pour un HAUT, choisis "
            "un produit de la MÊME saison; si le motif ressemble mais la saison "
            "diffère, réponds 0. EXCEPTION IMPORTANTE: pour un PANTALON, "
            "JOGGING, TRACKPANTS, SHORT ou des CHAUSSURES, IGNORE complètement "
            "la saison (ils se portent toute l'année) — ne rejette JAMAIS un "
            "pantalon/jogging à cause de sa saison [crochets].\n"
            ...
            "3) POUR UN ENSEMBLE (2 pièces), VÉRIFIE CHAQUE PIÈCE: le haut ET "
            "le bas doivent avoir la MÊME couleur que la photo. Si le haut "
            "match mais le BAS a une couleur différente (ex: photo = pantalon "
            "BORDEAUX, candidat = pantalon NOIR), CE N'EST PAS le même produit "
            "→ réponds 0.\n"
            "IMPORTANT — le produit de la photo n'est PAS forcément dans le "
            "catalogue. NE FORCE PAS un match: si aucun candidat n'a le MÊME "
            "type + motif + TOUTES les couleurs, réponds 0. Mieux vaut 0 "
            "(l'équipe vérifie) qu'un mauvais produit. Ne choisis un numéro que "
            "si tu es vraiment sûr que c'est le MÊME produit.\n"
            "Réponds UNIQUEMENT par le numéro (ex: 3). Réponds 0 si aucun "
            "produit ne colle vraiment (type + motif + toutes les couleurs)."
        )
